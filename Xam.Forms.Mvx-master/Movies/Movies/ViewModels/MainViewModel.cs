    ...
    public class MainViewModel :MvxViewModel
    {
       ...
       // ObservableCollection<quiz_by_circle> _tmpQuizzesList;
        ...
        void DoRefreshCommentsCommand()
        {
            ...
            if (CurrentLOIDSelected != CoolBeans.Configure.Constants.NoSelection)
                LoadLOComments(CurrentLOIDSelected);
        }
        ...
        {
            SharedPreferences prefs = CoolBeans.Configure.Constants.GetSharedPreferences(CoolBeans.Configure.Constants.PreferencesFileName);

            UserFirstName = prefs.GetString(CoolBeans.Configure.Constants.UserFirstNameKey);
            UserLastName = prefs.GetString(CoolBeans.Configure.Constants.UserLastNameKey);
            string image_url = prefs.GetString(CoolBeans.Configure.Constants.UserImageUrlKey);

            CacheService cache = CacheService.Init(SessionService.GetCredentialFileName(), CoolBeans.Configure.Constants.PreferencesFileName, CoolBeans.Configure.Constants.LocalDbName);
        ...
        async Task LoadLearningObjects(int circle_id)
        {
            ...
            CirclesLearningObjects[circleIndex] = new ObservableCollection<lo_by_circle_wrapper>(circleLOList);
             ...
             await BlockDownload.TryPutBytesInVector<MainViewModel.lo_by_circle_wrapper>(LearningOjectsList.ToList(),
        ...
        {
            if (lo_id != CoolBeans.Configure.Constants.NoSelection && _TmpQuizzesList != null)
            {
