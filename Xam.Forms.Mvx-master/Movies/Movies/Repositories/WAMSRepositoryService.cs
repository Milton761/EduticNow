...
namespace CoolBeans.Services
{
    public class WAMSRepositoryService : IRepositoryService
    {
        ...
        public WAMSRepositoryService(ISQLiteConnectionFactory factory)
        {
            ...
            _liteConnection = factory.Create(CoolBeans.Configure.Constants.LocalDbName);
        ...
        public async Task<List<T>> SearchForWithCacheAsync<T>(System.Linq.Expressions.Expression<Func<T, bool>> predicate, string identifier) where T : new()
        {
            ...
            CacheService service = CacheService.Init(SessionService.GetCredentialFileName(), CoolBeans.Configure.Constants.PreferencesFileName, CoolBeans.Configure.Constants.LocalDbName);
            try
            {
